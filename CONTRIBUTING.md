...
If you're unsure about any part of contributing, please get in touch. The best place is to start a new discussion on 
...
For these reasons (and others) it is important that every part of all software is documented as well as possible, 
...
The conventions for these repositories is as follows:
...
code style. You may need to run `pre-commit install` prior to working on the code to ensure that these tests are run 
...
All commits & pull requests will be build by GitHub actions 
([e.g. cellfinder](https://github.com/brainglobe/cellfinder/actions)). To ensure there are no issues, please check 
that it builds: `pip install .` and run all of the tests: `pytest` before committing changes.
...
Cellfinder includes a lot of functionality brought in from other packages. It is important that 
each repository focusses on a single task. Changes should be added to the relevant repository:
